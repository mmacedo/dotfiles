...
@recipes = %W{ less devise action_mailer add_user cleanup git extras html5 inherited_resources seed_database simple_form mongoid paperclip haml rspec home_page home_page_users users_page guard capybara resque }
...
after_bundler do
  say_wizard "Generating configuration for Metrical"
  create_file '.metrics' do
    <<-RUBY
MetricFu::Configuration.run do |config|
  ...
  say_wizard "Generating configuration for SimpleCov"
  create_file '.simplecov', "SimpleCov.start 'rails'"
  prepend_file 'test/test_helper.rb', "require 'simplecov'\n" if File.exists? 'test/test_helper.rb'
end
...
@current_recipe = "resque"
@before_configs["resque"].call if @before_configs["resque"]
say_recipe 'Resque'

config['resque'] = yes_wizard?("Would you like to manage brackground tasks with Resque?") if true && true unless config.key?('resque')
@configs[@current_recipe] = config
...
@current_recipe = "less"
@before_configs["less"].call if @before_configs["less"]
say_recipe 'Less CSS'
...
case HOST_OS
  ...
  after_bundler do
    ...
    get 'https://raw.github.com/livereload/livereload-js/master/dist/livereload.js', 'app/assets/jvascripts/livereload.js'
  end
